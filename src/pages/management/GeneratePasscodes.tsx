...
const GeneratePasscodes = () => {
  ...
  const syncWithSupabase = useAppState((s) => s.syncWithSupabase);
  ...
  useEffect(() => {
    syncWithSupabase().catch(err => {
      ...
      toast({ 
        title: "Sync failed", 
        description: "Could not sync with database. Using local data.",
        variant: "destructive" 
      });
    });
    ...
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => syncWithSupabase()} variant="outline" size="sm">
                    ...
                    {passcodes.slice(0, 5).map((passcode) => (
                      <div key={passcode.id} className="flex items-center justify-between p-2 border rounded">
                        <div className="text-sm">
                          ...
                          <div className="text-muted-foreground">{passcode.traineeEmail}</div>
                        </div>
